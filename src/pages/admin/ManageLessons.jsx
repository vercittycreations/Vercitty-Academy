...
const EMPTY_FORM = { title: '', description: '', youtubeUrl: '', resourceLink: '', order: 0 }
...
export default function ManageLessons() {
  ...
  const openEdit = (l) => {
    setEditLesson(l)
    setForm({ title: l.title, description: l.description || '', youtubeUrl: l.youtubeUrl || '', resourceLink: l.resourceLink || '', order: l.order || 0 })
    setError(''); setModal(true)
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editLesson) {
        ...
        setLessons(ls => ls.map(l => l.id === editLesson.id ? { ...l, ...payload } : l)
          .sort((a,b) => a.order - b.order))
      } else {
        const ref = await createLesson(payload)
        setLessons(ls => [...ls, { id: ref.id, ...payload }].sort((a,b) => a.order - b.order))
      }
      closeModal()
    } catch (err) {
  ...
  return (
    <PageWrapper>
      ...
      <div className="flex items-center gap-4 mb-2">
        <button onClick={() => navigate('/admin/courses')} className="flex items-center gap-2 text-dark-400 hover:text-white transition-colors text-sm">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-dark-400 text-sm mt-1">Add and manage lessons for this course.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {lessons.map((lesson, i) => {
            ...
            return (
              <div key={lesson.id} className="card border border-dark-800 p-4 flex items-center gap-4 hover:border-dark-700 transition-all">
                {/* Order badge */}
                ...
                <div className="w-24 h-14 rounded-lg overflow-hidden bg-dark-800 shrink-0">
                  {thumb
                    ...
                    : <div className="w-full h-full flex items-center justify-center"><PlayCircle size={18} className="text-dark-600" /></div>
                  }
                </div>
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    {lesson.youtubeUrl && (
                      <span className="inline-flex items-center gap-1 text-[10px] text-emerald-400">
                        <PlayCircle size={9} /> Video linked
                      </span>
                    )}
                    {lesson.resourceLink && (
                      <span className="inline-flex items-center gap-1 text-[10px] text-amber-400">
                        <FileText size={9} /> Resource linked
                      </span>
                    )}
                    {!lesson.youtubeUrl && !lesson.resourceLink && (
                      <span className="text-[10px] text-dark-600">No media linked</span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => openEdit(lesson)}      className="btn-secondary px-3 py-1.5 text-xs"><Pencil size={12}/> Edit</button>
                  <button onClick={() => setDeleteId(lesson.id)} className="btn-danger   px-3 py-1.5 text-xs"><Trash2 size={12}/></button>
                </div>
              </div>
            )
          })}
      ...
      {/* Create / Edit Modal */}
      <Modal open={modal} onClose={closeModal} title={editLesson ? 'Edit Lesson' : 'Add New Lesson'} width="max-w-xl">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <textarea rows={2} className="input resize-none" placeholder="What will students learn in this lesson?"
            ...
            <div className="col-span-2">
              <label className="label">Google Drive Resource Link</label>
              <div className="relative">
                <Link2 size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-dark-400 pointer-events-none" />
                <input type="url" className="input pl-9" placeholder="https://drive.google.com/..."
                  value={form.resourceLink} onChange={e => setForm(f => ({...f, resourceLink: e.target.value}))} />
              </div>
              <p className="text-xs text-dark-500 mt-1">Paste a Google Drive shareable link (PDF, ZIP, doc, etc).</p>
            </div>
            <div>
      ...
      <ConfirmDialog
        ...
        title="Delete Lesson"
        message="This will permanently delete this lesson and all student progress data for it."
      />
    </PageWrapper>
  )
}